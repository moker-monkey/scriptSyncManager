...
class Manager:
    ...
    def run(self, script_name: str, func_name: Optional[str] = None, 
            save_to_db: bool = True, verbose: bool = False) -> Dict[str, Any]:
        """
        运行指定脚本
    ...
    def test(self, script_name: str, func_name: Optional[str] = None, 
    ...
    def list(self, filter_type: str = "all", verbose: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if filter_type == "all":
                display_scripts = scripts_info['regular_scripts'] + scripts_info['test_scripts']
                title = "所有脚本"
            ...
            for i, script in enumerate(display_scripts, 1):
                ...
                if verbose:
                    ...
                    print(f"    修改时间: {script['modified_time'].strftime('%Y-%m-%d %H:%M:%S')}")
                    print()
    ...
    def _print_execution_result(self, result: Dict[str, Any], verbose: bool = False) -> None:
        """
        打印执行结果
        ...
        if verbose and result['result'] is not None:
            print("   结果详情:")
            if hasattr(result['result'], '__len__') and len(str(result['result'])) > 200:
                # 对于较长的结果，显示前200个字符
    ...
    def convert_menu(self, menu_path: str = None, verbose: bool = False) -> Dict[str, Any]:
        """
    ...
    def _print_convert_result(self, result: Dict[str, Any], verbose: bool = False) -> None:
        """
        打印转换结果
        ...
        if verbose and result['details']:
            ...
            for detail in result['details']:
                ...
                preserved = " (保留last_sync_datetime)" if detail.get('last_sync_preserved') else ""
                print(f"   {action_icon} {detail['script_name']} - {detail['action']}{preserved}")
...
使用示例:
  ...
  %(prog)s list                                    # 列出所有脚本
  %(prog)s list --filter test                      # 只显示测试脚本
  ...
  %(prog)s convert-menu -v                         # 转换 Menu.json 并显示详细信息
    ...
    run_parser.add_argument('--func', dest='func_name', help='要执行的函数名称（默认为main或run）')
    ...
    # list 命令
    list_parser = subparsers.add_parser('list', help='列出可用的脚本')
    list_parser.add_argument('--filter', choices=['all', 'regular', 'test'], 
                           default='all', help='脚本过滤类型')
    ...
    convert_menu_parser.add_argument('--menu-path', help='Menu.json 文件路径（默认为项目根目录的 Menu.json）')
    convert_menu_parser.add_argument('-v', '--verbose', action='store_true', help='显示详细信息')
